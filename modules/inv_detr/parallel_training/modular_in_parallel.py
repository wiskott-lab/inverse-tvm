...
if __name__ == '__main__':
    ...
    parser.add_argument("--inv_enc_id", "-ieid", help='experiment id of inv enc', type=str, default=None)
    parser.add_argument("--inv_dec_id", "-idid", help='experiment id of inv dec', type=str, default=None)
    parser.add_argument("--inv_detect_id", "-idet", help='experiment id of inv detector', type=str, default=None)
    ...
    if run_id:
        ...
        if inv_bb_id is not None:
            inv_bb = nu.init_model_from_run(run_id=inv_bb_id, module=inv_bb_module)
        else:
            inv_bb = nu.init_model_from_params(module=inv_bb_module, params=run_params)

        if inv_enc_id is not None:
            inv_enc = nu.init_model_from_run(run_id=inv_enc_id, module=inv_enc_module)
        else:
            inv_enc = nu.init_model_from_params(params=run_params, module=inv_enc_module)

        if inv_dec_id is not None:
            inv_dec = nu.init_model_from_run(run_id=inv_dec_id, module=inv_dec_module)
        else:
            inv_dec = nu.init_model_from_params(params=run_params, module=inv_dec_module)

        if inv_detect_id is not None:
            inv_detect = nu.init_model_from_run(run_id=inv_detect_id, module=inv_detector_module)
        else:
            inv_detect = nu.init_model_from_params(params=run_params, module=inv_detector_module)
