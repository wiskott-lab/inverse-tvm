import torch
import config
from modules.inv_detr.inv_bb import models as inverse_backbone_module
from modules.inv_detr.inv_bb.utils import test_inv_bb
from modules.detr.hubconf import detr_resnet50
import argparse
from torch.functional import F
from pathlib import Path
import uuid
from tools import detr_utils as du, training_utils as tu, logging_utils as nu, coco_utils as cu
...
if __name__ == '__main__':
    ...
    if run_id:
        ...
        inv_bb, inv_bb_optim = nu.init_from_checkpoint(checkpoint, inverse_backbone_module, run_params)
        best_loss = checkpoint['best_loss']
        test_step, train_step = checkpoint['test_step'], checkpoint['train_step']
        last_epoch = run_params['epochs']
        inv_bb.to(config.DEVICE)
    else:
        inv_bb = nu.init_model_from_params(module=inverse_backbone_module, params=run_params)
