...
if __name__ == '__main__':
    ...
    model_configs = {'inv_vit_bb': {'module_type': 'EnhancedInverseVitBackbonePos'}}
    optim_configs = {'inv_vit_bb': {'module_type': 'Adam', 'lr': lr}}
    ...
    if run_id:
        ...
        inv_bb, inv_bb_optim = nu.init_from_checkpoint(checkpoint, inv_vit_bb_module, run_params)
        best_loss = checkpoint['best_loss']
        test_step, train_step = checkpoint['test_step'], checkpoint['train_step']
        last_epoch = run_params['epochs']
        inv_bb.to(config.DEVICE)
    else:
        inv_bb = nu.init_model_from_params(module=inv_vit_bb_module, params=run_params)
        ...
        run = nu.init_run(project=config.PROJECT, source_files=[str(Path(__file__))], capture_hardware_metrics=False,
                               monitoring_namespace='monitoring', capture_stdout=False, capture_stderr=False)
        best_loss = test_inv_bb(inv_bb=inv_bb, run=run, vit=vit, dataloader=dataloader_val)
    ...
    for epoch in nu.progress(range(epochs), desc="epochs"):
        ...
        for batch_id, (tensor, _) in enumerate(nu.progress(dataloader_train, desc=f"epoch {epoch + 1}/{epochs}", leave=False)):
            ...
            train_step += 1
        loss = test_inv_bb(inv_bb=inv_bb, run=run, vit=vit, dataloader=dataloader_val)
        test_step += 1
        if loss < best_loss:
