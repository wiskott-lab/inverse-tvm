...
if __name__ == '__main__':
    ...
    parser.add_argument("--resnet_id", '-ri', type=str, default='detr50')
    ...
    resnet = detr.backbone[0].body
    resnet.act1 = resnet.relu
    input_proj = detr.input_proj
    resnet.eval().to(config.DEVICE)
    input_proj.eval().to(config.DEVICE)
    ...
    best_losses = test_inverse_models(inverse_models=inverse_models, dataloader=dataloader_val, forward_model=resnet, input_proj=input_proj, run=run)
    ...
    nu.upload_checkpoint_keys(models=inverse_models, optims=optims, best_loss=best_losses.tolist(), run=run,
                              test_step=test_step, train_step=train_step, keys=module_keys)
    ...
    for epoch in range(epochs):
        ...
        for batch_id, (nested_tensor, _) in enumerate(dataloader_train):
            ...
            with torch.no_grad():
                embs = ru.tensor_to_embs(model=resnet, tensor=img, input_proj=input_proj)
            ...
            train_step += 1
        eval_value = test_inverse_models(inverse_models=inverse_models, dataloader=dataloader_val, forward_model=resnet, input_proj=input_proj, run=run)
        test_step += 1
