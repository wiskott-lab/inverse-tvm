...
if __name__ == '__main__':
    ...
    nu.upload_checkpoint_keys(models=models, optims=optims, best_loss=best_losses.tolist(), run=run,
    ...
    for epoch in range(epochs):
        for batch_id, (img, _) in enumerate(dataloader_train):
            ...
            for i in range(len(recon_embs)):
                ...
                optims[i].zero_grad()
                optim_scalers[i](loss, optims[i], clip_grad=1.0, parameters=models[i].parameters())
                run[f"train/{str(i)}"].append(loss.item())
            train_step += 1
        eval_value = test_inverse_swin(models=models, dataloader=dataloader_val, model=swin, run=run)
        test_step += 1
        for i in range(len(eval_value)):
            if eval_value[i] < best_losses[i]:
                best_losses[i] = eval_value[i]
                nu.upload_model_state_key(model=models[i], run=run, key=str(i))
        nu.upload_checkpoint_keys(models=models, optims=optims, best_loss=best_losses.tolist(), run=run,
                                  test_step=test_step, train_step=train_step, keys=('0', '1', '2', '3', '4'))
