...
from modules.detr.util.misc import collate_fn, NestedTensor
from torchvision import transforms

coco_eval_strings = ['AP - IoU 0.5:0.95 - all - 100',
                     'AP - IoU 0.5 - all - 100',
                     'AP - IoU 0.75 - all - 100',
                     'AP - IoU 0.5:0.95 - small - 100',
                     'AP - IoU 0.5:0.95 - medium - 100',
                     'AP - IoU 0.5:0.95 - large - 100',
                     'AR - IoU 0.5:0.95 - all - 1',
                     'AR - IoU 0.5:0.95 - all - 10',
                     'AR - IoU 0.5:0.95 - all - 100',
                     'AR - IoU 0.5:0.95 - small - 100',
                     'AR - IoU 0.5:0.95 - medium - 100',
                     'AR - IoU 0.5:0.95 - large - 100']
...
vit_transforms = coco_transforms.Compose([coco_transforms.Resize(size=(224, 224)), coco_transforms.ToTensor(),
                                              coco_transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                        std=[0.229, 0.224, 0.225])])

coco_annotations_file = config.COCO_DIR / 'annotations' / 'instances_val2017.json'
...
def get_dataset(transform, return_mask, dataset_type):
    if dataset_type == 'val':
        ...
        dataset = DetrCocoDetection(img_folder=config.COCO_DIR / 'train2017',
                                    ann_file=config.COCO_DIR / 'annotations' / 'instances_train2017.json',
                                    transforms=transform, return_masks=return_mask)
    else:
...
def get_dataloader(transform=default_transforms, category_ids=(), img_ids=(), batch_size=1, return_mask=False,
    ...
    if len(category_ids) > 0 or len(img_ids) > 0:
        ...
        subset_indices = [id_to_index[img_id] for img_id in img_ids if img_id in id_to_index]
...
class RecolorObjectHUEImageNet:
    def __init__(self, color_filter, category=None, whiten_surroundings=False, single_object=False, add_filter=False):
        self.color_filter = color_filter
        self.category = category
        self.whiten_surroundings = whiten_surroundings
